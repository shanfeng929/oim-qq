...
public class CommonCache {
	...
	public Set<Serializable> getSet(String key) {
		Set<Serializable> set = redisTemplate.execute(new RedisCallback<Set<Serializable>>() {
			public Set<Serializable> doInRedis(RedisConnection connection) throws DataAccessException {
				Set<Serializable> resultSet = redisTemplate.opsForSet().members(key);
				if (resultSet == null) {
					resultSet = new HashSet<Serializable>();
				}
				return resultSet;
			}
		});
