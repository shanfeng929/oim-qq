...
import com.onlyxiahui.common.util.OnlyUUIDUtil;
...
public class UserCategoryService {
	...
	@Resource
	UserCategoryMemberPush userCategoryMemberPush;
	...
	public ResultMessage addUserCategoryMember(UserCategoryMember userCategoryMember) {
		...
		if (null != userCategoryMember && null != userCategoryMember.getMemberUserId()) {
			try {
				...
				String key = OnlyUUIDUtil.getUUID();
				userCategoryMemberPush.pushAddUserCategoryMember(key, userCategoryMember, userCategoryMember.getOwnUserId());

				String memberUserId = userCategoryMember.getOwnUserId();
				String ownUserId = userCategoryMember.getMemberUserId();

				List<UserCategory> list = userCategoryDAO.getListByUserId(ownUserId);
				if (null != list && !list.isEmpty()) {
					UserCategory uc = list.get(0);
					UserCategoryMember ucm = new UserCategoryMember();

					ucm.setMemberUserId(memberUserId);
					ucm.setOwnUserId(ownUserId);
					ucm.setUserCategoryId(uc.getId());

					userCategoryMemberDAO.save(ucm);

					userCategoryMemberPush.pushAddUserCategoryMember(key, ucm, ucm.getOwnUserId());
				}
			} catch (Exception e) {
				message.addWarning(ErrorConstants.warning_system + ".0001", "系统异常！");
			}
