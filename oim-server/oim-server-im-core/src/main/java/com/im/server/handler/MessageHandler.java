...
public class MessageHandler {
	...
	@Resource
	private QueueTaskThread queueTaskThread;
	...
	private void init() {
		vs = new ValidatorService("classpath*:/validator/im/*.json");
	}
	...
	public void doMessage(String message, SocketSession socketSession) {
		Object result = null;
		if (OnlyJsonUtil.mayBeJSON(message)) {
			try {
				...
				if (hasHead && hasBody) {
					...
					boolean validator = validator(path, head, bodyObject, socketSession);
					if (!validator) {
						return;
					}
	...
	private boolean validator(String path, Head head, JsonObject bodyObject, SocketSession socketSession) {
		...
		if (!list.isEmpty()) {
			...
			for (ValidatorResult vr : list) {
				m.addError(vr.getErrorCode(), vr.getPropertyPath() + vr.getMessage());
			}
			m.setHead(head);
			write(socketSession, m);
		}
